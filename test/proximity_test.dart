...
void main() {
  ...
  group('isTrustworthy', () {
    test('rejects a fix fuzzier than the accuracy cap', () {
      expect(isTrustworthy(fix(futala, acc: 51)), isFalse);
      expect(isTrustworthy(fix(futala, acc: 49)), isTrue);
    });
  ...
  group('evaluate', () {
    ...
    test('lists a place once inside the radius', () {
      final r = evaluate(at: north(futala, 31), all: all, unlocked: {});
    ...
    test('the boundary is inclusive and just outside it is not', () {
      expect(evaluate(at: north(futala, 49), all: all, unlocked: {}).inRadius, hasLength(1));
      expect(evaluate(at: north(futala, 51), all: all, unlocked: {}).inRadius, isEmpty);
    });
